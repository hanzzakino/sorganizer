...
export default function Navbar({theme, collapsed}) {
	...
	<div className={collapsed ? 'navbar-menulist-collapsed bottom-line flex':'navbar-menulist bottom-line flex'}>
		<span>
			...
			<button className={(collapsed ? theme+'-fgcolor navbar-menulist-btn-collapsed ':theme+'-fgcolor navbar-menulist-btn ')+(currentView==='subjects' ? 'navbar-btn-selected':'')} onClick={() => setDashboardView('subjects')}>Subjects</button>
		</span>
		<span>
			<i className={'bi bi-card-checklist navbar-menulist-btn-icon '+theme+'-fgcolor'} />
			<button className={(collapsed ?  theme+'-fgcolor navbar-menulist-btn-collapsed ': theme+'-fgcolor navbar-menulist-btn ')+(currentView==='todo' ? 'navbar-btn-selected':'')} onClick={() => setDashboardView('todo')}>To&#8211;do</button>
		</span>
		<span>
			<i className={'bi bi-calendar-week navbar-menulist-btn-icon '+theme+'-fgcolor'} />
			<button className={(collapsed ?  theme+'-fgcolor navbar-menulist-btn-collapsed ': theme+'-fgcolor navbar-menulist-btn ')+(currentView==='schedule' ? 'navbar-btn-selected':'')} onClick={() => setDashboardView('schedule')}>Schedule</button>
		</span>
		<span>
			<i className={'bi bi-journal navbar-menulist-btn-icon '+theme+'-fgcolor'} />
			<button className={(collapsed ?  theme+'-fgcolor navbar-menulist-btn-collapsed ': theme+'-fgcolor navbar-menulist-btn ')+(currentView==='notes' ? 'navbar-btn-selected':'')} onClick={() => setDashboardView('notes')}>Notes</button>
		</span>
	</div>
	<br />
	<div className={collapsed ? 'navbar-menulist-collapsed bottom-line flex':'navbar-menulist bottom-line flex'}>
		<span>
			...
			<button className={(collapsed ?  theme+'-fgcolor navbar-menulist-btn-collapsed ': theme+'-fgcolor navbar-menulist-btn ')+(currentView==='settings' ? 'navbar-btn-selected':'')} onClick={() => setDashboardView('settings')}>Settings</button>
		</span>
		<span>
